...
%         hold on;   
        ...
        text(lceNV(idxValid(1,1)),...
             fxeNV(idxValid(1,1)),...
             sprintf('%1.3f',fitInfo.fv.yErr(idx,1)),...
             'HorizontalAlignment','left',...
             'VerticalAlignment','top',...
             'FontSize',6);
        hold on;
        ...
        text(expTRSS2017.activeLengtheningData(idx).x(idxKeyF0)./lceOptMdl,...
             expTRSS2017.activeLengtheningData(idx).y(idxKeyF0),...
             sprintf('%1.3f',fitInfo.fl.yErr(idx,1)),...
             'HorizontalAlignment','left',...
             'VerticalAlignment','top',...
             'FontSize',6);
        hold on;
...
for idx=simConfig.trials     
    ...
    xErr = fitInfo.QToF.x(end,idx)./lceOptMdl;
    yErr = (fitInfo.QToF.y(end,idx) + fitInfo.QToF.yFit(end,idx)).*0.5;

    rmseTrial = sqrt(mean(fitInfo.QToF.yErr(:,idx).^2));
    if(idx==1)
        text(xErr,yErr,...
    ...
    for idxF = 1:1:length(fitInfo.QToF.x)
        x0 = fitInfo.QToF.x(idxF,idx) ./ lceOpt;
        x1 = x0;
        y0 = fitInfo.QToF.y(idxF,idx);        
        y1 = fitInfo.QToF.yFit(idxF,idx);
...
%          'FontSize',6);        
   ...
   if(idx==1)
        npts=100;
        x0 = benchRecordFitted.normFiberForce(1,idx);
        x1 = min(fitInfo.QToF.x(:,idx))./lceOptMdl;
        ...
        text(xV(idxMaxErr,1),yV(idxMaxErr,1)+0.25,...
             sprintf('%s%1.2f%s','$$\epsilon_{MAX}=',maxErrV,'$$'),...
             'HorizontalAlignment','right',...
             'FontSize',6);        
        hold on;
        plot([1,1].*xV(idxMaxErr,1),...
             ([0,0.2]+yV(idxMaxErr,1)),...
             '-k',...
             'HandleVisibility','off');
        hold on;
   end
